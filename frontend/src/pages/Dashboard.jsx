...
import { useSystemStore, useTaskStore, useAgentStore, useSocketStore, useThemeStore } from '../store';
import { useSocketEvents } from '../hooks/useSocketEvents';
import Sidebar from '../components/Sidebar';
import ConnectionStatus from '../components/ConnectionStatus';
import { Activity, Bot, FileText, AlertTriangle, Clock, Zap } from 'lucide-react';
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div className={`rounded-xl p-6 mb-6 border ${
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className={`text-xl font-semibold capitalize ${isDark ? 'text-white' : 'text-slate-900'}`}>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            <StatCard icon={<FileText size={20} />} label="Total Tasks" value={health?.tasks?.total || 0} color="primary" isDark={isDark} />
            <StatCard icon={<Activity size={20} />} label="Active Tasks" value={health?.tasks?.active || 0} color="cyan" isDark={isDark} />
            <StatCard icon={<Bot size={20} />} label="Active Agents" value={health?.agents?.active || 0} color="green" isDark={isDark} />
            <StatCard icon={<AlertTriangle size={20} />} label="Blocked" value={health?.tasks?.blocked || 0} color="amber" isDark={isDark} />
          </div>
            ...
            <div className={`rounded-xl border ${
              ...
              <div className={`divide-y ${isDark ? 'divide-slate-800' : 'divide-slate-200'}`}>
                {tasks.slice(0, 5).map((task) => (
                  <div key={task._id} className={`p-4 transition-colors ${
            ...
            <div className={`rounded-xl border ${
              ...
              <div className={`divide-y ${isDark ? 'divide-slate-800' : 'divide-slate-200'}`}>
                {agents.filter(a => a.status === 'active').slice(0, 5).map((agent) => (
                  <div key={agent._id} className={`p-4 transition-colors ${
                    isDark ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'
                  }`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`font-medium ${isDark ? 'text-white' : 'text-slate-900'}`}>{agent.name}</p>
                          <p className="text-sm text-slate-400">{agent.role}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className={`text-sm font-medium ${isDark ? 'text-white' : 'text-slate-900'}`}>{agent.load}%</div>
                        <div className={`h-1 w-16 rounded-full overflow-hidden ${
                          isDark ? 'bg-slate-800' : 'bg-slate-200'
                        }`}>
                          <div className="h-full bg-accent-cyan" style={{ width: `${agent.load}%` }} />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {agents.filter(a => a.status === 'active').length === 0 && (
                  <div className="p-8 text-center text-slate-400">No active agents</div>
                )}
              </div>
            </div>
          </div>
